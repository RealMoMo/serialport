 ...
 * @time 2020/10/13 09:53
 * @describe 更新音频版数据包
 *
 * 中间部分的有用信息 -> 版本，文件路径，文件的md5   中间用 ' | ' 分割
 */
public class UpgradeAudioVersionRecevierModel extends RecevierSerialPortModel {
...
//     */
    ...
    //4.提供AudioUpgrade数据类型
    //private static String OFFER_TYPE = "7E";
    //3+4
    private static final String DATA_TYPE = "0B7E";
    //5.数据的内容
    ...
    private AudioVersionUpgradeBean audioUpgradeBean;
     ...
     * @param context
     */
    public UpgradeAudioVersionRecevierModel(HHTDeviceManager hhtDeviceManager, String rawData, String dataHex, Context context) {
        ...
        if(split.length == 3){
            audioUpgradeBean = new AudioVersionUpgradeBean();
            audioUpgradeBean.setVersion(split[0]);
            audioUpgradeBean.setUpgradePackagePath(split[1]);
            audioUpgradeBean.setMd5(split[2]);
            audioUpgradeBean.setRecordTime(Calendar.getInstance(TimeZone.getTimeZone("Asia/Shanghai")).getTime().toString());
        }
        mContext = context;
    }
    ...
    public void action() {
        //通知Android系统，升级信息
        if(audioUpgradeBean == null){
            return ;
        }
        SharedPreferences sharedPreferences = mContext.getSharedPreferences(GlobalConfig.SP_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor edit = sharedPreferences.edit();
        edit.putString(GlobalConfig.KEY_SP_ADUIO_UPGRADE_INFO, audioUpgradeBean.parseToJSONObject().toString());
        edit.commit();
    ...
    private void putDataChangeMessage(){
        try {
            ...
            conValue.put(SerialPortDAO.NAME, SerialPortDAO.KEY_AUDIO_UPGRADE);
            conValue.put(SerialPortDAO.VALUE, "");
            mContext.getContentResolver().update(Uri.withAppendedPath(SerialPortDAO.SERIAL_PORT_URI, SerialPortDAO.KEY_AUDIO_UPGRADE), conValue, null, null);
